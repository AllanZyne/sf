...
Definition partition {X : Type}
  ...
  (* fold (fun x p => if test x 
    then (x::(fst p), snd p)
    else (fst p, x::(snd p))) l ([], []). *)
...
Lemma map_app_distr : forall (X Y : Type) (f : X -> Y) (l1 : list X) (l2 : list X),
...
Definition fold_partition {X : Type}
                          (test : X -> bool)
                          (l : list X)
                          : list X * list X :=
  fold (fun x p => if test x 
...
Proof.
  intros X l.
  induction l as [| n l' IHl'].
  - simpl. (* => fold_length [ ] = 0 *) reflexivity.
  - simpl. rewrite <- IHl'. reflexivity.
Qed.
(** [] *)
