...
Proof.
  ...
  split.
  - intros [HP | [HQ HR]].
    + assert (HPQ: P \/ Q). { apply or_intro. apply HP. }
      assert (HPR: P \/ R). { apply or_intro. apply HP. }
      apply and_intro.
      apply HPQ. apply HPR.
    + assert (HPQ: P \/ Q). { apply or_comm, or_intro, HQ. }
      assert (HPR: P \/ R). { apply or_comm, or_intro, HR. }
      apply and_intro.
      apply HPQ. apply HPR.
  - intros [[HP | HQ] [HP' | HR]].
    + apply or_intro, HP.
    + apply or_intro, HP.
    + apply or_intro, HP'.
    + assert (HQR: Q /\ R). { apply and_intro. apply HQ. apply HR. }
      apply or_comm, or_intro, HQR.
Qed.
(** [] *)
...
Proof.
  intros n m p.
  rewrite mult_0. (* (!) rewrite outside firstly *)
...
Proof.
  ...
  - intros [x [Hf HIn]].
    ...
    + simpl in HIn.
      simpl.
      destruct HIn as [Hhx | HInxl'].
      {
        left. rewrite Hhx. apply Hf.
      }
      {
        right. apply IHl'. apply HInxl'.
      }
Qed.
(** [] *)
...
Lemma rev_append_app_x : forall X (x:X) (l1 l2: list X), 
  rev_append l1 (l2++[x]) = rev_append l1 l2 ++ [x].
Proof.
  intros X x l1.
  induction l1 as [| h t IHl1].
  - simpl. reflexivity.
  - intros l2. simpl.
    apply IHl1 with (l2 := (h::l2)).
Qed.
...
Proof.
  ...
  - simpl.
    ...
    simpl.
    rewrite <- IHx.
    apply (rev_append_app_x) with (l2 := []).
Qed.
(** [] *)
...
Proof.
  ...
  - rewrite evenb_S. (* (!) (evenb n') destructable *)
...
Proof.
  intros [] b2.
  - split.
    + intros _. apply or_intro. reflexivity.
    + intros _. reflexivity.
  - split.
    + destruct b2.
      * intros _. apply or_comm. apply or_intro. reflexivity.
      * intros contra. discriminate contra.
    + destruct b2.
      * intros [contra | H].
        discriminate contra.
